...
<para>This calls `CallbackEventHandler.RegisterCallback{TEventType}(EventCallback{TEventType},TrickleDown)` on the same control (equivalent to registering a `ChangeEvent{T}` callback directly). `ChangeEvent{T}` participates in propagation; handlers on an ancestor receive bubbled events from descendant controls of the same event type.</para>
